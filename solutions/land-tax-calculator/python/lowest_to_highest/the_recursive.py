import sys


def lbtt_calc(house_price: float) -> float:
    if house_price == 0:  # recursive floor
        return 0
    bands_and_rates = [
    #     band    rate
        (145_000, 0.00),
        (250_000, 0.02),
        (325_000, 0.05),
        (750_000, 0.10),
        (sys.maxsize, 0.12),
    ]
    prev_band = 0
    for band, rate in bands_and_rates:
        if house_price <= band:
            tax_for_current_band = ((house_price - prev_band) * rate)
            tax_for_prev_bands = lbtt_calc(house_price=prev_band)
            return tax_for_current_band + tax_for_prev_bands
        prev_band = band


if __name__ == "__main__":
    assert lbtt_calc(house_price=1_000_000) == 78_350
    assert lbtt_calc(house_price=250_000) == 2_100
    assert lbtt_calc(house_price=145_000) == 0
